...
operation_data <- predictions_figures %>%
  filter(!is.na(fldb_vessel_id))%>% 
  dplyr::select(fldb_vessel_id,name=year,offender,class,mmsi)%>%
  mutate(mmsi=as.factor(mmsi))%>%
...
ilo_indicator_fig <- combined_case_data %>%
  ...
  mutate(indicator_type = case_when(indicator_type=="case_info"~"Case status information",
                                    ...
                                    TRUE ~ "Vessel operation status and model classification")) %>%
  mutate(indicator_type = fct_relevel(indicator_type,"ILO forced labor indicator")) %>%
  ggplot(aes(x=fldb_vessel_id,y=name,fill=class,color=offender)) +
  ...
  scale_color_manual("Training data label",values = c("red","black")) +
  scale_fill_manual("Model classification",values = c("gray30","gray80")) +
  guides(size = FALSE,
