...
operation_data <- vessel_operating_years %>%
  mutate(`Operating in 2019` = ifelse(year == 2019,TRUE,NA),
         `Operating in 2020`= ifelse(year == 2020,TRUE,NA)) %>%
  dplyr::select(-mmsi,-year,-in_operation) %>%
  pivot_longer(-fldb_vessel_id) %>%
  filter(!is.na(value)) %>%
  mutate(fldb_vessel_id = as.character(fldb_vessel_id)) %>%
  mutate(indicator_type = "operation_years")
...
operation_summary_data <- rev(c("Operating in 2019","Operating in 2020"))

summary_data <- c(ilo_summary_data$name,case_summary_data,operation_summary_data)
...
ilo_indicator_fig <- combined_case_data %>%
  ...
  ggplot(aes(x=fldb_vessel_id,y=name)) +
...
ggsave(here::here("output_figures/figure_s1.png"),ilo_indicator_fig,width=7.5,height=5,device="png",dpi=300)
```
