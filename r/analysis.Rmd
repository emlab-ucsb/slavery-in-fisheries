...
world_bbox <- data.frame(xmin = -180,
                         xmax = 180,
                         ymin = -90,
                         ymax=90)
...
map_figure_rel <-   grouped_messages %>%
  ...
  mutate(gear = case_when(gear=="drifting_longlines" ~ "(A) Longliner",
                          ...
                          TRUE ~ "(C) Trawler")) %>%
  ggplot() +
  geom_rect(data=world_bbox, aes(xmin=xmin , xmax=xmax, ymin=ymin, ymax=ymax ), color="grey30", fill="black")+
  geom_tile(aes(x=lon_bin,y=lat_bin,fill = fraction_at_risk_fishing_kW_hours,color=fraction_at_risk_fishing_kW_hours),size=0) +
 geom_polygon(data = world_land, fill = "grey60", color = "grey60",
              aes(x = long,
                 y = lat,
                group = group)) +
...
world_bbox_sf <- tibble(x = c(180,180,-180,-180,180),y=c(90,-90,-90,90,90)) %>% 
  as.matrix() %>% 
  list() %>% 
  st_polygon() %>%
  st_sfc(crs=4326)
