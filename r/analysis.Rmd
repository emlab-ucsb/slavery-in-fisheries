...
combined_case_data <- bind_rows(ilo_data,case_data)
...
summary_data <- c(ilo_summary_data$name,case_summary_data)
...
ilo_indicator_fig <- combined_case_data %>%
  ...
  mutate(indicator_type = ifelse(indicator_type=="case_info","Case status information","ILO forced labor indicator")) %>%
  mutate(indicator_type = fct_relevel(indicator_type,"ILO forced labor indicator")) %>%
  ggplot(aes(x=fldb_vessel_id,y=name)) +
  facet_col(facets = "indicator_type",scales="free",space="free") +
  geom_count() +
  theme_bw() +
