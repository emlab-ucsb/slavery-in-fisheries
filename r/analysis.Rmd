...
## generate_figures - If this is TRUE, all figures for the PNAS manuscript will be generated. This not only generates figures for this markdown notebook, but also caches the figures as .png files.This GitHub currently has cached figures as they were published in PNAS.
...
SELECT
  ...
  SUM(known_length_m) number_vessel_years_known_length,
  SUM(known_tonnage_gt) number_vessel_years_known_tonnage_gt,
  SUM(known_engine_power_kw) number_vessel_years_known_engine_power_kw,
  SUM(known_crew_size) number_vessel_years_known_crew_size,
  SUM(known_length_m)/COUNT(*) fraction_vessel_years_known_length,
  SUM(known_tonnage_gt)/COUNT(*) fraction_number_vessel_years_known_tonnage_gt,
  SUM(known_engine_power_kw)/COUNT(*) fraction_number_vessel_years_known_engine_power_kw,
  SUM(known_crew_size)/COUNT(*) fraction_number_vessel_years_known_crew_size
FROM
  joined"
...
write_csv(vessels_fig_data_summary_combined,here::here("interim_data/s12_figure_2_data.csv"))
...
still_in_operation_stats <- predictions_figures %>%
  ...
  summarize(count_in_operation = n(),
            fraction_in_operation = n()/length(unique(predictions_figures %>% filter(!is.na(fldb_vessel_id)) %>% .$fldb_vessel_id)),
            count_high_risk = sum(class),
            fraction_high_risk = sum(class)/count_in_operation)

operation_data <- predictions_figures %>%
  filter(!is.na(fldb_vessel_id)) %>%
  dplyr::select(fldb_vessel_id,name=year,offender,class)%>%
...
The model identifies `r prettyNum(n_predicted_positive_vessel_years_new,big.mark=",")` new high-risk vessel-years that were previously unlabeled.
...
For the training dataset used in this analysis, `r round(known_vessel_info$fraction_vessel_years_known_length[1]*100)`% of vessel-years have known vessel length from registries, `r round(known_vessel_info$fraction_number_vessel_years_known_tonnage_gt[1]*100)`% have known gross tonnage, `r round(known_vessel_info$fraction_number_vessel_years_known_engine_power_kw[1]*100)`% have known engine power, and `r round(known_vessel_info$fraction_number_vessel_years_known_crew_size[1]*100)`% have known crew size.
