...
vessels_fig_total <- predictions_figures_private  %>%
  filter(class == 1) %>%
...
vessels_fig_percentage <- predictions_figures_private %>%
  group_by(year,gear) %>%
...
#   group_by(mmsi,year,gear,offender,fldb_vessel_id) %>%
#   summarize(class = ifelse(sum(class)>=n()/2,1,0),
#             probability = mean(.pred_1)) %>%
#   ungroup()
# 
# predictions_figures_mapping <- robustness_predictions_figures %>%
#   filter(include_vessel_characteristics & year_assumption==1) %>%
#   dplyr::select(mmsi,year,gear,offender,fldb_vessel_id)
...
# For remaining analyis, just look at 2018
port_data <- port_data %>%
  filter(year==2018) %>%
  left_join(port_visit_summary,by= c("port_iso3"))

# How many countries had visits by known offenders in the 2012-2018 time frame
port_data%>%
...
# How many countries have high-risk visits, but no known offender visits
port_data %>%
