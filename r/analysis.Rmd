...
write_csv(grouped_messages,path=here::here("interim_data/grouped_messages.csv"))
write_csv(grouped_messages %>%
            filter(year==2018) %>%
            dplyr::select(-known_offender_fishing_kW_hours,
                  -fraction_known_offender_fishing_kW_hours) ,path=here::here("interim_data//spatial_fishing_risk_fig_3.csv"))
```

```{r}
grouped_messages <- read.csv(here::here("interim_data/grouped_messages.csv")) %>%
  filter(year == 2018)
...
write_csv(port_data,here::here("interim_data/port_data.csv"))
write_csv(port_data %>%
            filter(year==2018),path=here::here("interim_data/port_risk_fig_4.csv"))
```

```{r}
port_data <- read_csv(here::here("interim_data/port_data.csv")) %>%
  filter(year == 2018,
         !is.na(port_iso3))
