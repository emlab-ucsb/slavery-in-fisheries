...
# Read in cached data
training_df <- read_csv(here::here("raw_data/training_full.csv")) %>%
  filter(!is.na(flag)) %>%
...
# Number of bags
bag_vec <- tibble(bag = seq(5))
# Different models to try
model_type_vec <- tibble(model_type = c("random-forest","svm"))
tic()
# Define all model runs
model_runs <-cv_splits %>%
  # For each CV fold, try different downsampling ratios if bagging
  # Also perform a number of bags
  crossing(under_ratio_vec,
           bag_vec) %>%
  ...
  dplyr::select(-splits,-recipe)
toc()
# Initialize progress bar
pb <- progress_estimated(nrow(model_runs))

tic()
# Make predictions for all model runs
predictions <- model_runs %>%
  mutate(predict = pmap(list(x=workflow,y=analysis,z=assessment),function(x,y,z){
     # Fit analysis data using workflow
    tmp <- fit(x,y) %>%
    ...
    return(tmp)})) %>%
  # Select relevant columns
  dplyr::select(id,model_type,bagging,under_ratio,bag,predict) %>%
  unnest(predict)
toc()
# Save CV predictions
...
performance <- predictions %>%
  ...
  group_by(id,model_type,bagging,under_ratio,mmsi,year) %>%
  ...
  group_by(id,model_type,bagging,under_ratio) %>%
  nest() %>%
  ungroup() %>%
  ...
  group_by(model_type,bagging,under_ratio,.threshold,.metric) %>%
  summarize(mean_performance = mean(.estimate,na.rm=TRUE),
            sd_performance = sd(.estimate,na.rm=TRUE)) %>%
  ungroup() %>%
  group_by(model_type,bagging,under_ratio) %>%
  nest() %>%
  ungroup() %>%
...
toc()
# Save performance estimates for later
write_csv(performance,paste0(data_directory,"/tidy_cv_performance.csv"))
```

```{r}
# Read cached performance
performance <- read_csv(paste0(data_directory,"/tidy_cv_performance.csv"))
performance %>%
  ...
  mutate(under_ratio = ifelse(is.na(under_ratio),0,under_ratio)) %>%
  filter(.metric %in% c("recall","modified_f1")) %>% 
  ggplot(aes(x = factor(under_ratio),y=mean_performance,color=model_type)) + 
  geom_point(position=position_dodge(width=0.5)) +
  geom_errorbar(aes(ymin=error_min, ymax=error_max),position=position_dodge(width=0.5),width=0.5) + 
  facet_grid(.metric~.,scales="free") +
  theme_bw()

# Find optimal hyperparameter set that maximizes the modified F1 estimator
optimal_hyperparameters <- performance %>% 
  filter(bagging,under_ratio==1) %>%
  filter(.metric=="modified_f1") %>%
  arrange(desc(mean_performance)) %>%
  slice(1)
...
```{r}
bag_performance <- map_df(bag_vec$bag,function(x){
...
bag_performance <- read_csv(paste0(data_directory,"/tidy_cv_bag_performance.csv"))
cv_performance_fig <- bag_performance %>%
  mutate(number_bags = paste0("Number bags: ",number_bags)) %>%
  mutate(model_type=ifelse(model_type=="random-forest","Random forest","Support vector\nmachine")) %>%
  mutate(under_ratio = ifelse(is.na(under_ratio),"",under_ratio))  %>% 
  mutate(error_min = pmax(mean_performance-sd_performance,0),
         error_max = ifelse(.metric=="recall",pmin(mean_performance+sd_performance,1),mean_performance+sd_performance)) %>%
#  filter(.metric %in% c("recall","modified_f1")) %>% 
    mutate(.metric = case_when(.metric=="recall"~"Recall",
                               .metric=="modified_f1"~"Modified F1",
                               TRUE~"Detection prevalence")) %>%
  mutate(.metric = fct_relevel(.metric,"Recall","Detection prevalence")) %>%
  ggplot(aes(x = factor(under_ratio),y=mean_performance,color=model_type)) + 
  geom_point(position=position_dodge(width=0.75)) +
  geom_errorbar(aes(ymin=error_min, ymax=error_max),position=position_dodge(width=0.75),width=0.5) + 
  facet_grid(.metric~number_bags,scales="free") +
...
ggsave(here::here("output_figures/fig_s2.png"),cv_performance_fig,width=7.5,height=7.5,device="png",dpi=300)
...
cv_performance <- performance %>% 
  inner_join(optimized_model,by = c("model_type", "bagging", "under_ratio"))
...
tic()
# Define all model runs
model_runs_final <-crossing(under_ratio_vec,
                            bag_vec) %>%
  mutate(bagging = ifelse(is.na(under_ratio),FALSE,TRUE)) %>%
  # Don't need to bag if not doing downsampling
  filter(!(!bagging & bag >1)) %>%
...
predictions_final <- model_runs_final %>%
  mutate(fit = map(workflow,~fit(.x,training_df)),
         predict = map(fit,~predict(.x,training_df,type="prob") %>%
...
```{r}
predictions_final <- readRDS(data_directory,"/predictions_final.Rdata")
...
# Do google checks
check_positive <- predictions %>% 
  filter(class == 1 & offender ==0) %>% 
  arrange(desc(probability)) %>% 
  dplyr::select(mmsi,probability) %>% 
  # Add extra one since there is one multiple in there
  slice(1:51)

check_negative <- predictions %>% 
  filter(offender ==0) %>% 
  dplyr::select(mmsi,probability) %>% 
  sample_n(50)

#write_csv(check_positive,here::here("results/check_positive.csv"))

#write_csv(check_negative,here::here("results/check_negative.csv"))
```

## Variable importance
```{r}
vessel_lookup <- read_csv(here::here("data/indicator_lookup.csv"))
importance_fig <- predictions_final %>%
...
ggsave(here::here("output_figures/fig_s4.png"),importance_fig,width=7.5,height=5,device="png",dpi=300)
```
...
ggsave(here::here("output_figures/fig_s3.png"),class_fig,width=7.5,height=3.5,device="png",dpi=300)
```

## High risk vessels and crew over time

```{r}
number_high_risk_vessels <- predictions %>%
    filter(class ==1) %>%
    .$mmsi %>% unique() %>% length()
...
number_affected_crew <- predictions %>%
  filter(class ==1) %>%
  group_by(mmsi) %>%
...
flag_predictions_other <- predictions %>%
...
crew_vessels_fig <- crew_vessels_fig_data %>%
  ...
  facet_grid(indicator~gear,scales="free_y")+
...
ggsave(here::here("output_figures/fig_2.png"),crew_vessels_fig,width=7.5,height=5,device="png",dpi=300)
```
...
```{r}
training_df_figure <- read_csv(here::here("data/training_full.csv")) %>%
  filter(!is.na(flag)) %>%
  # Only include gears for which we have known offenders
  filter(gear %in% c("drifting_longlines","squid_jigger","trawlers")) %>%
  # Add unique id for every row
...
indicator_figure <- recipes::recipe(offender ~ ., data = head(training_df_figure)) %>%
  recipes::update_role(mmsi, new_role = "id variable") %>%
  ...
  scale_y_continuous(trans="asinh",breaks = scales::pretty_breaks(2)) +
    labs(
      x = "",
      y = "Box-Cox transformed, centered, and scaled\nindicator vales (inverse hyperbolic sin scale)"
    ) +
  theme_bw() +
...
ggsave(here::here("output_figures/fig_1.png"),indicator_figure,width=8,height=8,device="png",dpi=300)
```
...
training_dataset_fig <- training_df  %>%
  ...
  geom_bar(stat="identity",color="black",size=0.25) + 
  #coord_flip() + 
  facet_wrap(.~offender,scales="free",ncol=1)+
  disco::scale_fill_disco(palette = "bright", "Gear") +
  theme_bw() +
  labs(x="Year",y="Number of vessel-years")+
  scale_y_continuous(labels=scales::comma)+ 
  theme(strip.background =element_rect(fill=NA))

training_dataset_fig

ggsave(here::here("output_figures/fig_s1.png"),training_dataset_fig,width=7.5,height=4.5,device="png",dpi=300)
```
...
bq_table(project = ucsb_project,table = "risk_predictions",dataset = "human_rights") %>%
  bq_table_upload(values = predictions %>% dplyr::select(mmsi,year,offender,probability=.pred_1,class,gear) %>%
                    mutate(mmsi = as.numeric.factor(mmsi),
                           year = as.numeric.factor(year),
                           offender = as.numeric.factor(offender)),
...
write_csv(grouped_messages,path=paste0(data_directory,"/grouped_messages.csv"))
write_csv(grouped_messages %>%
            filter(year==2018) %>%
            dplyr::select(-known_offender_fishing_kW_hours,
                  -fraction_known_offender_fishing_kW_hours) ,path=paste0(data_directory,"/spatial_fishing_risk_fig_3.csv"))
```

```{r}
grouped_messages <- read.csv(paste0(data_directory,"/grouped_messages.csv")) %>%
  filter(year == 2018)
...
map_figure_rel
ggsave(here::here("output_figures/fig_3.png"),map_figure_rel,width=7.5,height=7.5,device="png",dpi=300)                                    
```
...
write_csv(port_data,paste0(data_directory,path="/port_data.csv"))
write_csv(port_data %>%
            filter(year==2018),path=paste0(data_directory,"/port_risk_fig_4.csv"))
```

```{r}
port_data <- read_csv(paste0(data_directory,path="/port_data.csv")) %>%
  filter(year == 2018,
         !is.na(port_iso3))
...
ggsave(paste0(data_directory,path="paper-figs/fig_4.png"),port_map_figure_rel,width=7.5,height=7.5,device="png",dpi=300)      
```

## Paper stats
```{r}
n_predicted_positive_vessel_years <- predictions %>%
...
n_predicted_positive_vessels <- predictions %>%
  filter(Prediction == "Positive") %>%
  distinct(mmsi) %>%
  nrow()

fraction_positive_vessels <- n_predicted_positive_vessels / length(unique(predictions$mmsi))

n_positive_crew <- predictions %>%
  filter(Prediction == "Positive") %>%
  distinct(mmsi,crew_size) %>%
  .$crew_size %>%
  sum()

mean_cv_recall <- cv_performance %>%
  filter(.metric=="recall") %>%
  .$mean_performance

correct_positives <- predictions %>%
  filter(Prediction == "Positive" & Label == "Positive") %>%
  nrow()

fraction_correct_positives <- correct_positives / (predictions %>%
  filter( Label == "Positive") %>%
  nrow())
```

# Greenpeace verification
...
sql<-"WITH
  ...
  year,
    ...
    best.best_flag flag,
    ais_identity.shipname_mostcommon.value shipname,
    ais_identity.n_shipname_mostcommon.value normalized_shipname,
    best.best_length_m length_m,
    best.best_tonnage_gt tonnage_gt,
    best.best_engine_power_kw engine_power_kw,
    best.best_crew_size crew_size
  FROM
...
bq_project_query(gfw_project, sql) %>%
  bq_table_download(max_results = Inf) %>%
  write_csv(path=here::here("interim_data/greenpeace_verification.csv"))
```


```{r}
# From paper
# 8 longliners, 6 of which we can match
# https://storage.googleapis.com/planet4-southeastasia-stateless/2019/12/b68e7b93-greenpeace-seabound-book-c.pdf
# Matching info: https://docs.google.com/spreadsheets/d/1L8bW-HpCEEQ5LwyJmnCboeAt6rOoxh9NefiirhRskM0/edit#gid=0
greenpeace_verification <- read_csv(here::here("interim_data/greenpeace_verification.csv"))

matched_boats <- greenpeace_verification %>%
  filter(mmsi %in% c(416236600,416001084,413322770,520313000) |
           imo %in% c(9260249,8996138,8748957))

n_matched_boats <- unique(matched_boats$mmsi) %>% length()
n_matched_vessel_years <- nrow(matched_boats)

performance <- matched_boats  %>%
  group_by(class) %>%
  summarize(count = n())

recall <- (performance %>%
  filter(class==1) %>%
  .$count) / nrow(matched_boats)

matched_high_risk_boats <- matched_boats %>% filter(class==1)

n_matched_high_risk_boats <- matched_high_risk_boats %>% .$mmsi %>% unique() %>% length()

high_risk_performance <- matched_boats %>%
  filter(mmsi %in% matched_high_risk_boats$mmsi)
```
