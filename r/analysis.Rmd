...
```{r}
predictions_final <- readRDS(paste0(data_directory,"/predictions_final.Rdata"))
...
# Do google checks
check_positive <- predictions_figures %>% 
...
check_negative <- predictions_figures %>% 
  filter(offender ==0) %>% 
  dplyr::select(mmsi,probability) %>% 
  sample_n(50)
...
## Variable importance
```{r}
vessel_lookup <- read_csv(here::here("raw_data/indicator_lookup.csv"))
...
crew_vessels_fig_data <- predictions_figures  %>%
  filter(fishing_hours > 0) %>%
  #filter(year == 2018) %>%
  filter(class == 1) %>%
  ...
  ungroup() %>%
  gather("indicator","count",-gear,-Country,-year) %>%
  mutate(indicator = fct_relevel(indicator,c("Number of vessels","Number of crew")))

write_csv(crew_vessels_fig_data,here::here("interim_data/crew_vessels_fig_data_fig_2.csv"))
...
```{r}
training_df_figure <- read_csv(here::here("raw_data/training_full.csv")) %>%
  filter(!is.na(flag)) %>%
  # Only include gears for which we have known offenders
  filter(gear %in% c("drifting_longlines","squid_jigger","trawlers")) %>%
  # Add unique id for every row
...
ggsave(here::here("/output_figures/fig_4.png"),port_map_figure_rel,width=7.5,height=7.5,device="png",dpi=300)      
```
