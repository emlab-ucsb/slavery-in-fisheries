...
# Create version of training dataset that only classified positive vessels for a single year, the year prior to being caught
# We will use this in a robustness check at the end of the analysis
training_df_reduced <- training_df %>%
  mutate(offender = ifelse(offender == 1 & years_until_caught == 1,1,0),
         offender = as.factor(offender)) %>%
  dplyr::select(-years_until_caught)

# Remove years_until_caught column now
...
## Stats for how often classification changes
...
sql<-"WITH
  ...
  SELECT
    ...
    class,
    year,
    LAG(year) OVER(PARTITION BY mmsi ORDER BY year ASC) previous_year,
    LAG(class) OVER(PARTITION BY mmsi ORDER BY year ASC) previous_class
  FROM
    `ucsb-gfw.human_rights.risk_predictions`),
  change_table AS(
  SELECT
    *,
    (CASE
        WHEN class = 1 AND previous_class = 0 THEN 'increased_risk'
        WHEN class = 0
      AND previous_class = 1 THEN 'decreased_risk'
      ELSE
      'no_change'
    END
      ) change
  FROM
    base_table
  WHERE
    NOT previous_class IS NULL)
SELECT
  change,
  COUNT(*) number_vessel_years,
  COUNT(*)/(
  SELECT
    COUNT(*)
  FROM
    change_table)*100 percentage_vessel_years
FROM
  change_table
GROUP BY
  change
...
SELECT
  ...
  SELECT
    COUNT(*)
  FROM
    change_table) number_vessel_years,
  100 percentage_vessel_years"

vessel_classification_change_info <- bq_project_query(gfw_project, sql) %>%
  bq_table_download(max_results = Inf)
```
...
```{r}
decreasing_risk_vessels <-predictions_figures %>%
  arrange(mmsi,year) %>%
  group_by(mmsi) %>%
  mutate(previous_class = lag(class),
         next_class = lead(class)) %>%
  ungroup() %>%
  filter((previous_class == 1 & class == 0) | (class == 1 & next_class == 0))

decreasing_risk_vessels_figure_data <- decreasing_risk_vessels %>%
  # Make binaries into 1s/0s
  mutate(foc = ifelse(foc,1,0),
         iuu = ifelse(iuu,1,0),
         ais_type = ifelse(ais_type == "A",1,0)) %>%
  dplyr::select(-year,-flag,-actual,-fldb_vessel_id,-source_id,-Country,-Prediction,-Label,-offender,-.pred_1,-previous_class,-next_class)

decreasing_risk_vessels_figure_data_processed <- recipes::recipe(class ~ ., data = head(decreasing_risk_vessels_figure_data)) %>%
  recipes::update_role(mmsi, new_role = "id variable") %>%
  recipes::update_role(gear, new_role = "id variable") %>%
  # Box-Cox transformation on all numeric
  recipes::step_BoxCox(all_numeric())%>%
  step_normalize(all_predictors()) %>%
  prep(decreasing_risk_vessels_figure_data) %>%
  juice() %>%
  pivot_longer(cols=c(-"mmsi",-"gear",-"class"),names_to = "indicator") %>%
  mutate(gear = case_when(gear=="drifting_longlines" ~ "Longliner",
                          gear=="squid_jigger" ~ "Squid jigger",
                          TRUE ~ "Trawler")) %>%
  left_join(vessel_lookup,by="indicator")

decreasing_risk_vessels_figure_data_processed %>%
  group_by(class,indicator_name,gear) %>%
  summarize(mean = mean(value,na.rm = TRUE)) %>%
  ungroup() %>%
  pivot_wider(names_from="class",values_from="mean") %>%
  mutate(delta = `0` - `1`) %>%
  filter(!is.nan(delta)) %>%
  ggplot(aes(x = reorder(indicator_name,delta),y=delta))+
  ...
  facet_grid(.~gear)  +
  labs(x = "",
       y = "Change in mean normalized indicator value\nwhen moving from high- to low-risk")

```

# Robustness check

Re-run the model, but train the model while assuming vessel-years are positive only in the single year prior to when the forced labor abuse was reported.

```{r eval = FALSE}
# Cross-validation
# Ensure there is no splitting across source_id (and consequently fldb_vessel_id) across analysis and assessment datasets
set.seed(101)
cv_splits <- group_vfold_cv(training_df_reduced, 
                            group = source_id,
                            v = 10)
...
model_runs <- crossing(under_ratio_vec,
  ...
  # Want different seed each time downsampling is done, for bagging
  mutate(seed = sample.int(10^5,n())) %>%
  # Only look at random forest and downsampling ratio of 1, the optimized model from the main analysis
  filter(model_type == "random-forest" & under_ratio ==1)
...
write_csv(predictions,paste0(data_directory,"/tidy_cv_predictions_100_robustness.csv"))
```

```{r eval = FALSE}
# Only look at 100 bags, the same number of bags as the main model
bags_to_show <- c(100)
predictions <- read_csv(paste0(data_directory,"/tidy_cv_predictions_100_robustness.csv"))
bag_performance <- map_df(bags_to_show,function(x){
  tmp_performance <- predictions %>%
    filter(bag<=x) %>%
    # For each fold and hyperparameter set, take average prediction for each vessel-year (mmsi and year) across bags
...
# Save bag performance estimates for later
write_csv(bag_performance,paste0(data_directory,"/tidy_cv_bag_performance_100_robustness.csv"))
```

```{r eval = FALSE}
# Read in cached bag performance
bag_performance <- read_csv(paste0(data_directory,"/tidy_cv_bag_performance_100_robustness.csv"))
...
predictions_final <- model_runs_final %>%
  ...
  dplyr::select(-seed,-recipe) %>%
  # For each workflow, do entire model fitting process across all folds
  mutate(fit = map(workflow,~fit(.x,training_df_reduced)),
         predict = map(fit,function(x){
           tmp <- x %>%
             # Predict assessment data using fit
             predict(training_df_reduced, type = "prob") %>%
             # Add predictions to assessment data
             bind_cols(training_df_reduced)%>%
             # Select relevant columns
             dplyr::select(mmsi,year,offender,.pred_1)
           # Increment progress bar
           pb$tick()$print()
...
saveRDS(predictions_final,paste0(data_directory,"/predictions_final_robustness.Rdata"))
...
# Save final predictions
write_csv(predictions_figures,here::here("interim_data/final_model_predictions_robustness.csv"))
```

```{r}
predictions_figures <- read_csv(here::here("interim_data/final_model_predictions_robustness.csv"))
# What is the number of unique high-risk vessels (vessels that were high-risk during at least 1 year)?
number_high_risk_vessels <- predictions_figures %>%
    filter(class ==1) %>%
    .$mmsi %>% unique() %>% length()
