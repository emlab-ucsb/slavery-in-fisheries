...
write_csv(bag_performance,here::here("interim_data/s7_figure_s2_data.csv"))
```

```{r}
# Read in cached bag performance
bag_performance <- read_csv(here::here("interim_data/s7_figure_s2_data.csv"))
...
write_csv(importance_data,here::here("interim_data/s8_figure_s4_data.csv"))
```
...
## Variable importance
```{r}
importance_fig <- read_csv(here::here("interim_data/s8_figure_s4_data.csv")) %>%
...
vessels_fig_percentage <- predictions_figures %>%
  ...
  theme(strip.background =element_rect(fill=NA),
        axis.text.x = element_text(angle = 90, hjust = 1))+
  labs(x="",y="Percentage of total vessels\nthat are high-risk")
