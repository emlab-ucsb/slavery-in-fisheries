...
ggsave(here::here("output_figures/figure_s2.png"),cv_performance_fig,width=7.5,height=7.5,device="png",dpi=300)
...
model_runs_final <- optimized_model %>%
  crossing(tibble(bag = seq(100))) %>% #seq(optimized_model$bagging))) %>%
  # Want different seed each time downsampling is done, for bagging
  mutate(seed = sample.int(10^5,n())) %>%
  mutate(recipe = map2(seed,bagging,function(x,y){
    # Add downsampling to pre-processing recipe if bagging
...
write_csv(predictions_figures,here::here("interim_data/final_model_predictions.csv"))


# Do google checks
# Check 50 vessels with highest risk score
# Found 1 new forced labor vessel during search
check_positive <- predictions_figures %>% 
  filter(class == 1 & offender ==0) %>% 
  arrange(desc(.pred_1)) %>% 
  dplyr::select(mmsi,.pred_1,year) %>%
  slice(1:50)
set.seed(101)
# Check 50 vessels at random that were classified as low-risk
check_negative <- predictions_figures %>% 
  filter(offender ==0) %>% 
  dplyr::select(mmsi,.pred_1,year) %>% 
  sample_n(50)

write_csv(check_positive,here::here("interim_data/check_positive.csv"))
write_csv(check_negative,here::here("interim_data/check_negative.csv"))
...
write_csv(importance_data,here::here("interim_data/figure_s4_data.csv"))
```

# Make figures

```{r}
predictions_figures <- reead_csv(here::here("interim_data/final_model_predictions.csv"))
```

## Variable importance
```{r}
importance_fig <- read_csv(here::here("interim_data/figure_s4_data.csv")) %>%
...
ggsave(here::here("output_figures/figure_s4.png"),importance_fig,width=7.5,height=5,device="png",dpi=300)
```
...
ggsave(here::here("output_figures/figure_s3.png"),class_fig,width=7.5,height=3.5,device="png",dpi=300)
```
...
write_csv(crew_vessels_fig_data,here::here("interim_data/figure_2_data.csv"))
...
ggsave(here::here("output_figures/figure_2.png"),crew_vessels_fig,width=7.5,height=5,device="png",dpi=300)
```
...
ggsave(here::here("output_figures/figure_s1.png"),training_dataset_fig,width=7.5,height=4.5,device="png",dpi=300)
```
...
write_csv(grouped_messages %>%
            ...
            dplyr::select(-known_offender_fishing_kW_hours,
                  -fraction_known_offender_fishing_kW_hours) ,path=here::here("interim_data/figure_3_data.csv"))
```

```{r}
grouped_messages <- read.csv(here::here("interim_data/figure_3_data.csv"))
...
write_csv(port_data %>%
            filter(year==2018),path=here::here("interim_data/figure_4_data.csv"))
```

```{r}
port_data <- read_csv(here::here("interim_data/figure_4_data.csv")) %>%
