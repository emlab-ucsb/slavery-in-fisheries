...
vessels_fig_total <- predictions_figures  %>%
  ...
  scale_x_discrete(breaks = unique(predictions_figures$year)) +
  theme(strip.background =element_rect(fill=NA),
...
training_df_figure <- read_csv(here::here("raw_data/s1_training_full.csv")) %>%
  ...
  mutate(offender = ifelse(offender,1,0),
         ...
         foc = ifelse(foc,1,0),
         iuu = ifelse(iuu,1,0))%>%
  # Only classify known offenders as positie in the year prior to being caught
  mutate(offender = ifelse(offender == 1 & years_until_caught == 1,1,0)) %>%
...
bq_table(project = ucsb_project,table = "risk_predictions",dataset = "human_rights") %>%
  bq_table_upload(values = predictions_figures %>% dplyr::select(mmsi,year,offender,probability=.pred_1,class,gear,fldb_vessel_id),
...
fraction_countries_at_risk <- n_countries_at_risk / countries
...
WITH
  ...
  SELECT
    ...
    TRUE in_operation
  FROM
    `world-fishing-827.gfw_research.vi_ssvid_byyear_v20200701`)
SELECT
  *
FROM
  vessel_info
...
bq_project_query(gfw_project, sql) %>%
  bq_table_download(max_results = Inf) %>%
  write_csv(here::here("interim_data/s9_vessel_operating_years.csv"))
```
...
fldb_data <-read_csv(here::here("interim_data/s3_suspected_forced_labor_database.csv"))%>%
  # Filter to just those cases that were used in the analysis
  filter(fldb_vessel_id %in% (training_df %>% filter(offender==1) %>% .$fldb_vessel_id))

vessel_operating_years <- read_csv(here::here("interim_data/s9_vessel_operating_years.csv"))

# Summarize the number and fraction of forced labor vessels in operation during each number
still_in_operation_stats <- vessel_operating_years %>%
  group_by(year) %>%
  summarize(count_in_operation = n(),
            fraction_in_operation = n()/length(unique(vessel_operating_years$fldb_vessel_id)))

operation_data <- vessel_operating_years %>%
  mutate(year = as.character(year),
         fldb_vessel_id = as.character(fldb_vessel_id))%>%
  left_join(training_df %>% 
              filter(offender==1) %>%
              dplyr::select(fldb_vessel_id,year,offender),
            by = c("fldb_vessel_id","year")) %>%
  dplyr::select(fldb_vessel_id,name=year,value=in_operation,offender)%>%
...
case_data <- fldb_data %>%
  dplyr::select(fldb_vessel_id,eyewitness_account,arrests_made,charges_filed,conviction_made,penalties_sanctioned) %>%
...
summary_data <- c(ilo_summary_data$name,case_summary_data,rev(sort(unique(vessel_operating_years$year))))

ilo_indicator_fig <- combined_case_data %>%
  mutate(`Training data label` = case_when(name == "2020" | name == "2019" ~ "Not included in training",
                                           is.na(offender) ~ "Unlabaled",
                                           TRUE ~ "Positive")) %>%
  mutate(`Training data label` = fct_relevel(`Training data label`,c("Positive","Unlabaled"))) %>%
  ...
  mutate(indicator_type = case_when(indicator_type=="case_info"~"Case status information",
                                    indicator_type=="ilo_indicator" ~ "ILO forced labor indicator",
                                    TRUE ~ "Vessel operation status")) %>%
  mutate(indicator_type = fct_relevel(indicator_type,"ILO forced labor indicator")) %>%
  ggplot(aes(x=fldb_vessel_id,y=name,fill=`Training data label`)) +
  ...
  scale_fill_manual(values = c("gray30","gray80","white")) +
  guides(size = FALSE,
