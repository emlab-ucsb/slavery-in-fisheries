...
bag_performance <- map_df(bags_to_show,function(x){
  ...
  mutate(offender = relevel(as.factor(offender),"1")) %>%
    ...
    # Get mean and SD of performance across folds
    # Don't use na.rm=TRUE so that thresholds with undefined F1 scores are ignored from finding the optimal
    # These would represent thresholds that could generate a detection prevalence of 0
